...
(function (a, b) {
	...
	function a(a) {
		var g = a._util;
		g.elements = z(a.options);
		g.count = g.elements.length;
		g.destroyed &&
			((g.destroyed = !1),
			a.options.container &&
				r(a.options.container, function (a) {
					k(a, 'scroll', g.validateT);
				}),
			k(window, 'resize', g.saveViewportOffsetT),
			k(window, 'resize', g.validateT),
			k(window, 'scroll', g.validateT));
		b(a);
	}
	function b(a) {
		for (var g = a._util, c = 0; c < g.count; c++) {
			...
			e.container && p && (l = l.closest(e.containerClass))
	...
	function e(a, b, c) {
		if (!u(a, c.successClass) && (b || c.loadInvisible || (0 < a.offsetWidth && 0 < a.offsetHeight)))
			if ((b = a.getAttribute(v) || a.getAttribute(c.src))) {
				...
				var g = b[w && 1 < b.length ? 1 : 0],
					d = a.getAttribute(c.srcset),
					e = 'img' === a.nodeName.toLowerCase(),
					A = (b = a.parentNode) && 'picture' === b.nodeName.toLowerCase();
				if (e || void 0 === a.src) {
					var p = new Image(),
						m = function () {
						...
						n = function () {
							e ? A || h(a, g, d) : (a.style.backgroundImage = 'url("' + g + '")');
							f(a, c);
							q(p, 'load', n);
							q(p, 'error', m);
						};
				...
				'video' === a.nodeName.toLowerCase()
					? (r(a.getElementsByTagName('source'), function (a) {
					  ...
					  a.load(),
					  f(a, c))
					: (c.error && c.error(a, 'missing'), y(a, c.errorClass));
	}
	function f(a, b) {
		y(a, b.successClass);
	...
	function n(a) {
		m.bottom = (window.innerHeight || document.documentElement.clientHeight) + a;
		m.right = (window.innerWidth || document.documentElement.clientWidth) + a;
	}
	function k(a, b, c) {
		a.attachEvent
	...
	return function (d) {
		...
		p = Element.prototype.closest;
		w = 1 < window.devicePixelRatio;
		m = {};
		m.top = 0 - c.options.offset;
		m.left = 0 - c.options.offset;
		...
		c.destroy = function () {
			...
			this.options.container &&
				r(this.options.container, function (b) {
					q(b, 'scroll', a.validateT);
				});
			q(window, 'scroll', a.validateT);
			q(window, 'resize', a.validateT);
			q(window, 'resize', a.saveViewportOffsetT);
			a.count = 0;
			a.elements.length = 0;
			a.destroyed = !0;
		};
		...
		n(c.options.offset);
		r(c.options.breakpoints, function (a) {
...
function rplg_load_imgs(a) {
	...
	for (var b = 0; b < a.length; b++)
		a[b].setAttribute('src', a[b].getAttribute('data-src')), a[b].removeAttribute('data-src');
}
function rplg_next_reviews(a, b) {
	...
	for (var f = 0; f < b && f < reviews.length; f++)
		reviews[f] && ((reviews[f].className = reviews[f].className.replace(a + '-hide', ' ')), rplg_load_imgs(reviews[f]));
	reviews = d.querySelectorAll(e);
...
function _rplg_popup(a, b, d, e, f, h) {
	...
	return window.open(
		a,
		'',
		'location=1,status=1,resizable=yes,width=' + b + ',height=' + d + ',top=' + f + ',left=' + h
	);
}
function _rplg_timeago(a) {
	for (var b = 0; b < a.length; b++) {
		...
		-1 < d.indexOf('google')
			...
			: (d =
					-1 < d.indexOf('facebook')
						? new Date(a[b].getAttribute('data-time').replace(/\+\d+$/, '')).getTime()
						: new Date(a[b].getAttribute('data-time').replace(/ /, 'T')).getTime());
...
function _grw_init_slider(a) {
	function b(b) {
		for (var e, f = a.querySelectorAll('.grw-row'), w = 0; w < f.length; w++) {
			var c = f[w],
				h = c.offsetWidth;
			e = 510 > h ? 'xs' : 750 > h ? 'x' : 1100 > h ? 's' : 1450 > h ? 'm' : 1800 > h ? 'l' : 'xl';
			c.className = 'grw-row grw-row-' + e;
		}
		k.length &&
			q != e &&
			setTimeout(function () {
				n.scrollLeft != b * k[0].offsetWidth && (n.scrollLeft = b * k[0].offsetWidth);
				d();
				q = e;
			}, 200);
	}
	function d() {
		var b = a.querySelector('.grw-reviews'),
			d = a.querySelectorAll('.grw-review'),
			g = Math.round(b.offsetWidth / d[0].offsetWidth),
			h = Math.ceil(d.length / g),
			c = a.querySelector('.grw-dots');
		if (c) {
			c.innerHTML = '';
			for (var k = 0; k < h; k++) {
				var l = document.createElement('div');
				l.className = 'grw-dot';
				x = Math.ceil((((b.scrollLeft + (b.scrollLeft + d[0].offsetWidth * g)) / 2) * h) / b.scrollWidth);
				x == k + 1 && (l.className = 'grw-dot active');
				l.setAttribute('data-index', k + 1);
				l.setAttribute('data-visible', g);
				c.appendChild(l);
				l.onclick = function () {
					var b = a.querySelector('.grw-dot.active');
					b = parseInt(b.getAttribute('data-index'));
					var c = parseInt(this.getAttribute('data-index')),
						d = parseInt(this.getAttribute('data-visible'));
					b < c ? f(d * Math.abs(c - b)) : e(d * Math.abs(c - b));
					a.querySelector('.grw-dot.active').className = 'grw-dot';
	...
	function e(a) {
		n.scrollBy(-k[0].offsetWidth * a, 0);
	}
	function f(a) {
		n.scrollBy(k[0].offsetWidth * a, 0);
	}
	function h() {
		var b = a.querySelector('.grw-review:last-child'),
			d = b.getBoundingClientRect();
		b = b.parentNode.getBoundingClientRect();
		(2 > Math.abs(b.left - d.left) || b.left <= d.left) &&
		d.left < b.right &&
		(2 > Math.abs(b.right - d.right) || b.right >= d.right) &&
		d.right > b.left
			? e(k.length - Math.round(n.offsetWidth / k[0].offsetWidth))
			: f(1);
		t = setTimeout(h, z);
	}
	var u = a.querySelector('.grw-row'),
		y = JSON.parse(u.getAttribute('data-options')),
		z = 1e3 * y.speed,
		n = a.querySelector('.grw-reviews'),
		k = a.querySelectorAll('.grw-review'),
		q = '',
		r = null,
		t = null,
		v = function () {
			(u.offsetWidth || u.offsetHeight || u.getClientRects().length) &&
			'hidden' !== window.getComputedStyle(u).visibility
				? (setTimeout(b, 1), _rplg_init_blazy(10), k.length && y.autoplay && setTimeout(h, z))
				: setTimeout(v, 300);
		};
	v();
	window.addEventListener('resize', function () {
		var a = Math.round(n.scrollLeft / k[0].offsetWidth);
		clearTimeout(r);
		r = setTimeout(b, 150, a);
	});
	n &&
	...
	var m = a.querySelector('.grw-prev');
	m &&
		(m.onclick = function () {
			e(1);
			t && clearInterval(t);
		});
	if ((m = a.querySelector('.grw-next')))
		m.onclick = function () {
			f(1);
			t && clearInterval(t);
		};
...
function grw_init(a, b) {
	...
	!(a = _rplg_get_parent(a, 'wp-gr')) ||
		'true' == a.getAttribute('data-exec') ||
		('slider' != b && 'grid' != b) ||
		(a.setAttribute('data-exec', 'true'), _grw_init_slider(a));
}
